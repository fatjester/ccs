...
- **2026-04-10**: **#944** `ccs update` now targets the currently running CCS install instead of whichever package manager also has `@kaitranntt/ccs` somewhere on the machine. CCS now detects the owner and prefix of the active binary from the install path, routes `npm`/`yarn`/`pnpm`/`bun` updates back through that same install root, verifies that the active install actually changed, and shows an exact manual recovery command when another global copy was updated instead.
